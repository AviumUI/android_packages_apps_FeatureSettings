...
class CategorySettingsFragment : SettingsBasePreferenceFragment() {
    ...
    private var keyboxFilePickerLauncher: ActivityResultLauncher<Intent>? = null
    ...
    private fun bindKeyboxPreferences() {
        ...
        keyboxClearPref?.setOnPreferenceClickListener {
            ...
            Toast.makeText(requireContext(), getString(R.string.keybox_toast_file_cleared), Toast.LENGTH_SHORT).show()
            updateKeyboxPreferences()
            true
        }
    }
    ...
    private fun handleKeyboxFileSelected(uri: Uri?) {
        ...
        try {
            cr.openInputStream(uri).use { inputStream ->
                ...
                Toast.makeText(ctx, getString(R.string.keybox_toast_file_loaded), Toast.LENGTH_SHORT).show()
                updateKeyboxPreferences()
            }
        } catch (e: IOException) {
